# Real-time Vietnam Airlines Price Tracker

Tool nay duoc dieu chinh theo workflow reverse engineering API cua Vietnam Airlines: bat Fetch/XHR trong DevTools, lay endpoint JSON, roi cho Python worker goi truc tiep moi 60 giay thay vi mo trinh duyet tu dong.

## Kien truc Docker Compose

- `db-service`: PostgreSQL luu luong nong de dashboard doc nhanh
- `scraper-service`: worker Python async goi endpoint VNA moi 60 giay
...
4. Tim request tra ve JSON co danh sach gia, flight number, departure time, fare class.
5. Copy cac thong tin sau vao `.env`:
   ...
   - `VNA_BEARER_TOKEN` va `VNA_SESSION_ID` neu request can token hoac session
...
- `FUEL_SCHEDULE_MODE=daily` de chay 1 lan/ngay hoac `hourly` de chay theo gio
...
- Parser dang dung heuristic de tim `flight_number`, `departure_time`, `fare_class`, `price` trong JSON. Neu payload VNA co schema co dinh, ban co the siet lai parser de giam duplicate.
